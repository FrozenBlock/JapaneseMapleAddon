...
By contributing to Japanese Maples - Wilder Wild, you agree with the [Developer Certificate of Origin (DCO)][DCO].
...
Use American English to keep consistency across Japanese Maples - Wilder Wild.

If there is more than one acceptable spelling of the same word, first check if one word is already
being used in Japanese Maples - Wilder Wild, FrozenLib, or in Mojang's Mappings, and if not, use the spelling that is most commonly used.
...
It allows to separate the actual name of the variable and the namespace, `wilderWildJapaneseMaple`.

Fields marked as `@Unique` must be prefixed with `wilderWildJapaneseMaple$` or `WILDERWILD_JAPANESE_MAPLE$` if the field is static and final.

Methods marked with `@Unique` must be prefixed with `wilderWildJapaneseMaple$`.

Injector or modifier methods do not need to be prefixed. Fabric takes care of that.

## Licensing & DCO

Japanese Maples - Wilder Wild is licensed under [FrozenBlock Modding Oasis License][LICENSE], and it has a [Developer Certificate of Origin (DCO)][DCO], which you are required to agree with to contribute.
...
[LICENSE]: ./LICENSE.md "Japanese Maples - Wilder Wild license file"
